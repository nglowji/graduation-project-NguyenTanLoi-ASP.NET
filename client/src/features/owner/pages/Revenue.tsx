...
import { 
  ...
  BarChart3, PieChart, Download, Wallet, CreditCard,
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 gap-8 animate-in fade-in duration-700">
        ...
        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">Calculating financial metrics...</p>
      </div>
    );
  }
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-600">Financial Suite</span>
          </div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Doanh thu & Tài chính</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Theo dõi hiệu quả kinh doanh và dòng tiền từ các sân bóng.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-[2.5rem] p-10 shadow-sm group hover:shadow-xl transition-all">
          <div className="flex items-center justify-between mb-12">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Xu hướng doanh thu</h3>
              </div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-13">Biến động dòng tiền theo thời gian</p>
            </div>
            <div className="flex items-center gap-2">
              {['7D', '1M', '1Y'].map(t => (
                <button key={t} className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all ${t === '1M' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}>{t}</button>
              ))}
            </div>
          </div>
          ...
          <div className="h-80 flex flex-col items-center justify-center border-2 border-dashed border-slate-100 dark:border-slate-700/50 rounded-[2rem] bg-slate-50/50 dark:bg-slate-900/30 group-hover:bg-white dark:group-hover:bg-slate-800 transition-colors">
            ...
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">Aggregating transaction data...</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-[2.5rem] p-10 shadow-sm group hover:shadow-xl transition-all">
          <div className="flex items-center justify-between mb-12">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-600 border border-emerald-100 dark:border-emerald-500/20">
                  <PieChart size={20} />
                </div>
                <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Cơ cấu doanh thu</h3>
              </div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-13">Phân bổ theo loại sân & dịch vụ</p>
            </div>
          ...
          <div className="h-80 flex flex-col items-center justify-center border-2 border-dashed border-slate-100 dark:border-slate-700/50 rounded-[2rem] bg-slate-50/50 dark:bg-slate-900/30 group-hover:bg-white dark:group-hover:bg-slate-800 transition-colors">
            <div className="relative w-40 h-40 mb-6">
              ...
              <div className="absolute inset-0 border-8 border-blue-600 rounded-full border-t-transparent border-l-transparent rotate-45" />
            </div>
