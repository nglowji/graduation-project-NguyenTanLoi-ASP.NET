...
import { Activity, Award, CalendarDays, CreditCard, Download, Filter, Loader2, Medal, Package, TrendingUp, Trophy, Wallet } from 'lucide-react';
  ...
  return <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="mx-auto flex max-w-[1500px] flex-col gap-5 overflow-hidden pb-16">
    <header className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.22em] text-blue-600">Báo cáo vận hành</p>
          <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950">Doanh thu sân</h1>
          <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-500">Theo dõi tiền thu, đơn đặt và hiệu quả từng loại sân trong kỳ đã chọn.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="rounded-xl bg-slate-50 px-4 py-3 ring-1 ring-slate-200">
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Tổng thu trong kỳ</p>
            <p className="mt-1 text-2xl font-black text-slate-950">{money(total)}</p>
          </div>
          <button className="inline-flex h-11 w-fit items-center gap-2 rounded-xl bg-blue-700 px-4 text-xs font-black text-white transition hover:bg-blue-800"><Download size={16} />Xuất báo cáo</button>
        </div>
      </div>
    </header>    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    ...
    <section className="grid min-w-0 gap-5 xl:grid-cols-[1.35fr_0.65fr]">
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-black">Trạng thái đơn</h2><p className="mt-1 text-xs font-bold text-slate-400">Phân bổ booking trong kỳ</p><div className="mt-6 space-y-5">{statuses.map((item, index) => <div key={item.status}><div className="mb-2 flex justify-between gap-3 text-xs font-black"><span>{statusLabel(item.status)}</span><span className="shrink-0 text-slate-400">{item.count} đơn</span></div><div className="h-2.5 overflow-hidden rounded-full bg-slate-100"><motion.div initial={{ width: 0 }} animate={{ width: `${item.count / maxStatus * 100}%` }} className={`h-full rounded-full ${bars[index % bars.length]}`} /></div></div>)}</div></article>
    </section>
